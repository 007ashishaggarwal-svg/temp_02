...
def export_all_intelligence(vault_dir=None):
    w_dir = vault_dir or WORKSPACE
    xlsx_p = os.path.join(w_dir, "RSSFeedChecker_Master_Guide_and_Data.xlsx")
    if not os.path.exists(xlsx_p):
        print(f"Notice: {xlsx_p} not found.")
        return
...
def clean_html_tags(text):
    ...
    for r in range(2, ws.max_row + 1):
        ...
        item = {
            ...
            "desk": str(row_vals[7] or "Corporate Strategy & M&A").strip(),
            "matched_keywords": str(row_vals[8] or "--").strip(),
            "source_name": str(row_vals[9] or "Newsroom").strip(),
            "source_classification": str(row_vals[10] or "1. News Aggregator & Trade Press").strip(),
            "article_url": str(row_vals[11] or "").strip(),
            "snippet": clean_html_tags(raw_snippet),
            "cluster_id": str(row_vals[13] or "").strip(),
            "cluster_hint": hint_str,
            "discovery_pillar": str(row_vals[15] or "").strip(),
    ...
    out_json = os.path.join(w_dir, "web", "data", "latest_intelligence.json")
    out_js = os.path.join(w_dir, "web", "data", "latest_intelligence.js")
